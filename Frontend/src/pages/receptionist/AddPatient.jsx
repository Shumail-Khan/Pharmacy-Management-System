...
const AddPatient = () => {
  ...
  // Mock data for patients (to track existing IDs)
  ...
  useEffect(() => {
    ...
    // Load existing patients from localStorage or initialize empty
    const storedPatients = localStorage.getItem('mock_patients');
    if (storedPatients) {
      setExistingPatients(JSON.parse(storedPatients));
    }
  }, []);
            ...
            <div class="token-body">
              ...
              <div class="info-section">
                ...
                <div class="info-row">
                  ...
                  <span class="info-value">${data.patient.mobile}</span>
                </div>
                <div class="info-row">
                  <span class="info-label">Date:</span>
                  <span class="info-value">${new Date(data.appointment.date).toLocaleDateString('en-PK')}</span>
                </div>
                <div class="info-row">
                  <span class="info-label">Time:</span>
                  <span class="info-value">${data.appointment.time}</span>
                </div>
              </div>
              ...
              <div class="doctor-info">
                <div class="info-row" style="border-bottom: none;">
                  ...
                  <span class="info-value">Dr. ${data.doctor.name}</span>
                </div>
                <div class="info-row" style="border-bottom: none; margin-top: 8px;">
                  <span class="info-label">📋 Specialization:</span>
                  <span class="info-value">${data.doctor.specialization}</span>
                </div>
              </div>

              <div class="payment-box">
                <div class="label">💰 Payment Received</div>
                <div class="amount">Rs. ${parseInt(data.paymentAmount).toLocaleString()}</div>
              </div>
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      try {
        // Step 1: Create new patient with mock data
        const newPatient = {
        ...
        // Step 2: Create appointment if selected
        if (createAppointmentNow && appointment.doctorId) {
          ...
          // Store generated data for printing
          setGeneratedData({
            tokenNumber: tokenNumber,
    ...
    <div className="flex bg-gray-50 min-h-screen">
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-6">
                ...
                <div className="border-t pt-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <label className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        checked={createAppointmentNow}
                        onChange={(e) => setCreateAppointmentNow(e.target.checked)}
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                          ...
                          <div className="relative">
                            ...
                            <input
                              ...
                              onChange={handleAppointmentChange}
                              placeholder="Auto-fills from doctor fee"
                              required={createAppointmentNow}
                              className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                            />
                          </div>
                          <p className="text-xs text-gray-500 mt-1">Fee automatically set from selected doctor</p>
                        </div>
                      </div>
                      ...
                      <div className="bg-blue-50 p-3 rounded-lg">
                        <p className="text-sm text-gray-600 flex items-center gap-2">
                          ...
                          Token number will be generated automatically after registration
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {!createAppointmentNow && (
                    <div className="bg-yellow-50 p-4 rounded-lg">
                      <p className="text-sm text-yellow-800 flex items-center gap-2">
                        ...
                        You can create an appointment later from the Appointments section
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-4 pt-4 border-t">
                  <button
                    type="submit"
                    disabled={loading}
                    className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-lg transition font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  >
                    {loading ? (
                      <>
                      ...
                      <>
                        ...
                        {createAppointmentNow ? "Register & Generate Token" : "Register Patient"}
                      </>
                    )}
                  </button>
                  <button
      ...
      {showSuccessModal && generatedData && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full animate-in slide-in-from-bottom duration-300">
            ...
            <div className="p-6">
              <div className="bg-orange-50 rounded-lg p-6 mb-6 text-center">
                ...
                <div className="mt-4 space-y-2 text-left bg-white p-3 rounded-lg">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-gray-800">{generatedData.patient.name}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">Doctor:</span>
                    <span className="font-medium text-gray-800">Dr. {generatedData.doctor.name}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">Payment:</span>
                    <span className="font-medium text-green-600">Rs. {parseInt(generatedData.paymentAmount).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">Date & Time:</span>
                    <span className="font-medium text-gray-800">{new Date(generatedData.appointment.date).toLocaleDateString()} at {generatedData.appointment.time}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2.5 rounded-lg transition flex items-center justify-center gap-2"
                >
